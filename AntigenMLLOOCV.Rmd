---
title: "R Notebook for Antigen ML project (using LOOCV) using the party package"
...
ggplot(df, aes(x=PC1, y=PC2, color=qPCR_PA), alpha(1/10)) + geom_point() + xlab("PC1 (70.9% variance explained)") +
  ylab("PC2 (27.5% variance explanined)")
...
ggplot(df2, aes(x=PC1, y=PC2, color=qPCR_PA), alpha(1/10)) + geom_point() + xlab("PC1 (73.7.9% variance explained)") +
  ylab("PC2 (18.0% variance explanined)")

```

```{r}
#classification using the party package using concentration to predict presence/absence of infection
library(party)
library(readr)
library(dplyr)
...
ggplot(df, aes(x=PC1, y=PC2, color=`Infection Level`), alpha(1/10)) + geom_point() + xlab("PC1 (70.9% variance explained)") +
  ylab("PC2 (27.5% variance explanined)") + scale_color_hue(breaks=c("none", "lowest", "low", "mid", "high"), 
...
df <- data.frame(fv$infection, pca.tbl$x[,1], pca.tbl$x[,2])
colnames(df) <- c("Infection Level", "PC1", "PC2")
ggplot(df, aes(x=PC1, y=PC2, color=`Infection Level`), alpha(1/10)) + geom_point() + xlab("PC1 (73.7% variance explained)") +
  ylab("PC2 (18.0% variance explanined)") + scale_color_hue(breaks=c("none", "lowest", "low", "mid", "high"), 
                                                            labels=c("none", "lowest", "low", "mid", "high"))
...
```{r}
#PCAs for concentration and logconcentration using new categorical cutoff values
...
fv <- mutate(fv, 
             infection = if_else(qPCR > 0 & qPCR <= 20, "lowest",
                         if_else(qPCR > 20 & qPCR <= 200, "low",
                         if_else(qPCR > 200 & qPCR <= 1500, "mid",
                         if_else(qPCR > 1500, "high", "none")))))
fv$infection <- factor(fv$infection)
fv$qPCR <- NULL
...
```{r}
#PCA using concentration and logconcentration without LDH
library(readr)
library(dplyr)
library(ggplot2)
...
```{r}
#PCA without LDH using infection level categories
#using concentration
library(party)
library(readr)
library(dplyr)
