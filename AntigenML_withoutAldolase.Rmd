---
title: "AntigenML_withoutAldolase"
...
tbl2 <- select(tbl, concentration_HRP2, concentration_pLDH, logconcentration_HRP2, logconcentration_pLDH,
               RDT, qPCR, fever, country, age)
tbl2 <- filter(tbl2, concentration_HRP2 != "NA", concentration_pLDH != "NA")
...
ggplot(df, aes(x=PC1, y=PC2, color=qPCR_PA), alpha(1/10)) + geom_point() + xlab("PC1 (65.1% variance explained)") +
  ylab("PC2 (34.9% variance explanined)")
...
ggplot(df2, aes(x=PC1, y=PC2, color=qPCR_PA), alpha(1/10)) + geom_point() + xlab("PC1 (73.6% variance explained)") +
  ylab("PC2 (26.4% variance explanined)")
```
...
fv <- select(tbl, concentration_HRP2, concentration_pLDH, qPCR)
fv <- filter(fv, concentration_HRP2 != "NA", concentration_pLDH != "NA", qPCR != "NA")
...
#logconcentration fv
fv.log <- select(tbl, logconcentration_HRP2, logconcentration_pLDH, qPCR)
fv.log <- filter(fv.log, logconcentration_HRP2 != "NA", logconcentration_pLDH != "NA", qPCR != "NA")
...
ggplot(df, aes(x=PC1, y=PC2, color=`Infection Level`), alpha(1/10)) + geom_point() + xlab("PC1 (65.1% variance explained)") +
  ylab("PC2 (34.9% variance explanined)") + scale_color_hue(breaks=c("none", "lowest", "low", "mid", "high"), 
...
df <- data.frame(fv$infection, pca.tbl$x[,1], pca.tbl$x[,2])
colnames(df) <- c("Infection Level", "PC1", "PC2")
ggplot(df, aes(x=PC1, y=PC2, color=`Infection Level`), alpha(1/10)) + geom_point() + xlab("PC1 (73.6% variance explained)") +
  ylab("PC2 (26.4% variance explanined)") + scale_color_hue(breaks=c("none", "lowest", "low", "mid", "high"), 
                                                            labels=c("none", "lowest", "low", "mid", "high"))
```
...
fv <- select(tbl, logconcentration_HRP2, logconcentration_pLDH, qPCR)
fv$qPCR_PA <- if_else(fv$qPCR > 0, "positive", "negative")
fv$qPCR_PA <- factor(fv$qPCR_PA)
fv <- filter(fv, logconcentration_HRP2 != "NA", logconcentration_pLDH != "NA", qPCR != "NA")
fv$qPCR <- NULL
fv$qPCR_PA <- factor(fv$qPCR_PA)
